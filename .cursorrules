...
These are custom project workflows, not built-in commands. The `/t.` prefix stands for "temus-tools". When you see `/t.deploy dev`, open `.temus-tools/commands/deploy.md` and follow the steps.

## Context Files

Before any task, read these files for project context:
- `.temus-tools/project.config` — project settings
- `.temus-tools/stack.md` — coding conventions (Next.js, TypeScript, Tailwind CSS, static export)
